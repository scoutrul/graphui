...
export default class InfoTable extends Component {
	...
	handlerChangeText = searchText => {
		const filteredData = [...this.sort(this.props.tableData.values, this.state.sortedIndex, this.state.ASC)];
		const sortedData = filteredData.filter(obj => {
			let find = 0;
			obj.forEach(v => {
	...
	handlerClearText = () => this.setState({
		searchText: "",
		sortedData: this.sort(this.props.tableData.values, this.state.sortedIndex, this.state.ASC)
	});
	...
	handlerDialogHide = () => this.setState({
		dialogVisible: false,
		name: '',
		type: '',
		id: null,
	...
	render() {
		...
		return <Card tableCard>
			...
			<DataTable baseId="import-source-list" {...tableSettings}>
				<Header headers={this.props.tableData.headers[0]}
